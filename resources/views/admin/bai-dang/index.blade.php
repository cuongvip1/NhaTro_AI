...
@section('content')
  ...
  <form method="GET" action="{{ route('admin.posts') }}" class="mb-4 bg-white p-4 rounded-lg shadow-sm border border-gray-100">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
      <div>
        <label class="text-sm text-gray-600 block mb-1">Tìm kiếm</label>
        <input type="text" name="q" value="{{ request()->get('q') }}" class="w-full px-3 py-2 border rounded" placeholder="tiêu đề, mô tả...">
      </div>

      <div>
        <label class="text-sm text-gray-600 block mb-1">Chủ trọ</label>
        <select name="chu_tro" class="w-full px-3 py-2 border rounded">
          <option value="">-- Tất cả --</option>
          @if(isset($owners))
            @foreach($owners as $owner)
              <option value="{{ $owner->id }}" {{ (string)request()->get('chu_tro') === (string)$owner->id ? 'selected' : '' }}>{{ $owner->ho_ten }}</option>
            @endforeach
          @endif
        </select>
      </div>
      ...
      <div>
        <label class="text-sm text-gray-600 block mb-1">Địa chỉ</label>
        <select name="dia_chi" class="w-full px-3 py-2 border rounded">
          <option value="">-- Tất cả --</option>
          @if(isset($regions))
            @foreach($regions as $r)
              <option value="{{ $r->ten_dia_chi }}" {{ request()->get('dia_chi') === $r->ten_dia_chi ? 'selected' : '' }}>{{ $r->ten_dia_chi }}</option>
            @endforeach
          @endif
        </select>
      </div>
      ...
      <div class="md:col-span-1">
        <label class="text-sm text-gray-600 block mb-1">&nbsp;</label>
        <div class="flex items-center space-x-2">
          <button type="submit" class="px-3 py-2 bg-indigo-600 text-white rounded">Lọc</button>
          <a href="{{ route('admin.posts') }}" class="px-3 py-2 border rounded text-sm">Reset</a>
        </div>
      </div>
    </div>
  </form>
  ...
  <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <tbody class="divide-y">
        @if(isset($posts) && isset($posts['data']) && count($posts['data']) > 0)
          ...
          <tr class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3">{{ $post['dia_chi'] ?? ($post['phong']['day_tro']['dia_chi'] ?? 'N/A') }}</td>
            <td class="px-4 py-3">
              @php
    ...
    @if(isset($posts) && isset($posts['total']) && $posts['total'] > 0)
      <div class="px-4 py-3 border-t bg-gray-50 text-sm text-gray-600 flex items-center justify-between">
        <div>Hiển thị {{ count($posts['data']) }} / {{ $posts['total'] }} bài viết</div>
        @if(isset($posts['current_page']) && isset($posts['last_page']))
          <div class="flex items-center space-x-3">
            ...
            <div class="text-gray-700">Trang {{ $posts['current_page'] }} / {{ $posts['last_page'] }}</div>
